...
# Example DataFrame with prices and corresponding time intervals (1-minute intervals assumed)
data = {
    'prices': [100, 102, 101, 105, 103, 107, 106, 108, 107, 110,
               109, 111, 110, 115, 113, 114, 112, 116, 117, 115,
               114, 113, 112, 111, 113, 115, 117, 116, 118, 119]
}

df = pd.DataFrame(data)


# Define a function to calculate the weighted average of peaks
def weighted_average_recent_peaks(prices):
    # Identify peaks using the find_peaks function from scipy
    peaks, _ = find_peaks(prices)

    # Filter peaks to only include those within the last 30 time points
    recent_peaks = peaks[peaks >= len(prices) - 30]
    ...
    weights = [(i - (max_index - 30)) + 1 for i in recent_peaks]

    # Normalize weights so the sum equals 1
...
# Calculate the weighted average price for recent 30 minutes of peaks
weighted_avg_price = weighted_average_recent_peaks(df['prices'])

print(f"Weighted Average Price for Recent 30 Minutes of Peaks: {weighted_avg_price}")
